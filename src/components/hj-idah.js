...
import Logo  from '../assets/hj-idah.svg';
...
const HjIdah = () => {
  ...
  const title = intl.formatMessage({ id: 'hjIdah' });
  // useEffect(()=> {
  ...
  const handleSubmit = (e) => {
    setLoad(true)
    e.preventDefault()
    axios.post(intl.formatMessage({ id: 'downloadUrl' }) + 'hj-idah/card.php', param, {
    ...
    .then(response => {
      ...
      a.href = url
      a.download = intl.formatMessage({ id: 'downloadHjIdah' })
      a.click()
      a.remove()
    ...
    <Fragment>
      ...
      <div className="block">        
        <div style={{ cursor: 'pointer' }} onClick={handleDivClick}>
          <img className="img-logo" width={180} src={Logo} alt="Toko Hj. Idah" />
          {load && <><span className="spinner"><Spinner size={64} sizeUnit={'px'} color={'#d17702'} /></span>
