...
import Logo from '../assets/toko-awaliya.svg';
...
const TokoAwaliya = () => {
  ...
  const title = intl.formatMessage({ id: 'tokoAwaliya' });
  // useEffect(()=> {
  ...
  const handleSubmit = (e) => {
    setLoad(true)
    e.preventDefault()
    axios.post(intl.formatMessage({ id: 'downloadUrl' }) + 'awaliya/card.php', param, {
    ...
    .then(response => {
      ...
      a.href = url
      a.download = intl.formatMessage({ id: 'downloadAwaliya' })
      a.click()
      a.remove()
    ...
    <Fragment>
      ...
      <div className="block">
        <div style={{ cursor: 'pointer' }} onClick={handleDivClick}>            
          ...
          {load && <><span className="spinner"><Spinner size={64} sizeUnit={'px'} color={'#970000'} /></span>
