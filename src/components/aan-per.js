...
import Logo from '../assets/aan-per.svg';
...
const AanPer = () => {
  ...
  const title = intl.formatMessage({ id: 'aanPer' });
  // useEffect(()=> {
  ...
  const handleSubmit = (e) => {
    setLoad(true)
    e.preventDefault()
    axios.post(intl.formatMessage({ id: 'downloadUrl' }) + 'toko-aan-per/card.php', param, {
    ...
    .then(response => {
      ...
      a.href = url
      a.download = intl.formatMessage({ id: 'downloadAanPer' })
      a.click()
      a.remove()
    ...
    <Fragment>
      ...
      <div className="block">        
        <div style={{ cursor: 'pointer' }} onClick={handleDivClick}>
          <img className="img-logo" width={180} src={Logo} alt="Toko Aan Perabotan" />
