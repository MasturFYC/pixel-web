...
import Logo from '../assets/toko-aan.svg';
...
const TokoAan = () => {
  ...
  const title = intl.formatMessage({ id: 'tokoAan' });
  // useEffect(()=> {
  ...
  const handleSubmit = (e) => {
    setLoad(true)
    e.preventDefault()
    axios.post(intl.formatMessage({ id: 'downloadUrl' }) + 'toko-aan/card.php', param, {
    ...
    .then(response => {
      ...
      a.href = url
      a.download = intl.formatMessage({ id: 'downloadTokoAan' })
      a.click()
      a.remove()
    ...
    <Fragment>
      ...
      <div className="block">        
        <div style={{ cursor: 'pointer' }} onClick={handleDivClick}>
          <img className="img-logo" width={180} src={Logo} alt="Toko Aan" />
