...
import Logo  from '../assets/toko-syauqi.svg';
...
const TokoSyauqi = () => {
  ...
  const title = intl.formatMessage({ id: 'tokoSyauqi' });
  // useEffect(()=> {
  ...
  const handleSubmit = (e) => {
    setLoad(true)
    e.preventDefault()
    axios.post(intl.formatMessage({ id: 'downloadUrl' }) + 'toko-syauqi/card.php', param, {
    ...
    .then(response => {
      ...
      a.href = url
      a.download = intl.formatMessage({ id: 'downloadSyauqi' })
      a.click()
      a.remove()
    ...
    <Fragment>
      ...
      <div className="block">        
        <div style={{ cursor: 'pointer' }} onClick={handleDivClick}>
          <img className="img-logo" width={180} src={Logo} alt="Toko Syauqi" />
